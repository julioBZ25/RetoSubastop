...
import { selectViewMode } from './inventorySelectors';
	...
	return (
		<div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
			<div className="flex flex-1 items-center gap-2">
				...
				<select
					className="cursor-pointer h-9 rounded-md border border-dashsuba-border bg-white px-3 text-sm focus:outline-none focus:ring-1 focus:ring-dashsuba-accent"
	...
	if (isLoading) {
		return (
			<div className="py-20 flex justify-center">
				<Loader2 className="h-8 w-8 animate-spin text-dashsuba-secondary" />
			</div>
		);
	}
	if (isError) {
		return <div className="py-20 text-center text-dashsuba-error">Error loading products.</div>;
	}
							...
							{data?.products.map((product: Product) => (
								<TableRow key={product.id}>
									...
									<TableCell>
										...
										<div className="text-xs text-dashsuba-secondary truncate max-w-[200px]">
											{product.description}
										</div>
									</TableCell>
									<TableCell>
		...
		<>
			<div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
				{data?.products.map((product: Product) => (
					<Card
						key={product.id}
						className="overflow-hidden group hover:border-dashsuba-secondary transition-all"
					>
						...
						<CardContent className="p-4">
							<div className="mb-1 text-[10px] font-bold text-dashsuba-secondary uppercase tracking-widest">
								{product.category}
							</div>
		...
		<Suspense
			fallback={
				<div className="py-20 text-center">
					<Loader2 className="h-8 w-8 animate-spin mx-auto text-dashsuba-secondary" />
				</div>
			}
		>
			<div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
				<div className="flex flex-col gap-1">
					<h1 className="text-3xl font-bold tracking-tight text-dashsuba-primary">
						Inventory Management
					</h1>
					<p className="text-lg text-dashsuba-secondary">
						Track your products, stock levels, and pricing in real-time.
					</p>
				</div>
